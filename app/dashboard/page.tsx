...
export default function AdminDashboard() {
  ...
  const counts = propertyType.reduce((acc, item) => {
    const key = item.propertyType ?? "Unknown";
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});

  // 2) Convert into desired array
  const propertyTypeData = Object.entries(counts).map(
    ([name, value], index) => ({
      name,
      value,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="bg-card">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Monthly inquiries over the last 6 months
            </p>
          </CardHeader>
          <CardContent>
            <div className="h-64">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="bg-card">
          <CardHeader>
            <CardTitle className="color-foreground">
      ...
      <div
        className={`grid grid-cols-1 ${
          hasAdminRole ? "lg:grid-cols-2" : "lg:grid-cols-1"
        }  gap-8`}
      >
        {/* Quick Actions */}
        {hasAdminRole && (
